module SlotMachine
  class IsKindOf < Instruction
    def initialize(object_class, klass, true_label: nil, false_label: nil)
      ...
      raise "object_class: expected Slotted, given #{@object_class}" unless @object_class.is_a?(Slotted)
    ...
    def to_risc(compiler)
      object_class = @object_class.to_register(compiler, self).known_type(:Class).set_compiler(compiler)
      klass = @klass
